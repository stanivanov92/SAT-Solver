...
public class SimulatedAnnealingSolver {
    ...
    public SimulatedAnnealingSolver(int[][] listVar, int[][] arrayClauses, int[][] originalArray,
            int[] listPureVariables, int[] listPureClause) {
    ...
    public String launchSolver(int tries, double maxTemp, double minTemp, double decayRate, int iterations,
            boolean varselection, boolean pureLiteral) {
    ...
    private void generateRandomAssignments() {
        for (int i = 1; i < listVar.length; i++) {
            // If the variables has been assigned true, increase the true literal values in
            // every clause that has this variable
    ...
    private void printSatisfyingAssignment() {
        System.out.println("\nWe have successfully found a solution after: " + flips + " flips with ratio: "
                + (double) flips / attempts + " and attempts: " + attempts + " at temp: " + temp);
