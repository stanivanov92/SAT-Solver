...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (fileName != null) {
            ...
            /*
             * } else if (args != null && args.length == 2) { File fl = new
             * File(projectDirectory + "/resources/" + args[0]); try { parser = new
             * Parser(fl, Integer.parseInt(args[1])); } catch (Exception e) {
             * e.printStackTrace(); }
             * 
             */
        } else {
            System.out.println(
                    "Please provide a cnf file, which should be located within the resource folder of the project.");
            return;
        }

        // SimulatedAnnealing sa = new
        // SimulatedAnnealing(parser.getVariablesList(),parser.getClausesList());
        SimulatedAnnealingSolver saSolver = new SimulatedAnnealingSolver(parser.getVariablesList(),
                parser.getClausesList());
        // for( int j = 0; j < 10; j++){
        ...
        String fName = maxTemp + "-" + minTemp + " - " + decayRate + " - " + iterationsPerCycle + "-" + strFileName
                + ".txt";
        ...
        for (int i = 0; i < 5; i++) {
            ...
            dataWriter.writeText(i + 1 + ". "
                    + saSolver.launchSolver(25, maxTemp, minTemp, decayRate, iterationsPerCycle, true, true));
